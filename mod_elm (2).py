#!/usr/bin/env python
import mod_globals
import sys, re, time, string, threading, socket
...
class ELM:
    ...
    def loadDump(self, dumpname):
        ...
        dumpname = os.path.join(mod_globals.user_data_dir, dumpname)
        df = open(dumpname, 'rt')
        lines = df.readlines()
        df.close()
    ...
    def monitor(self, callback, send_allow, c_t=0.1, c_f=10):
        ...
        while self.run_allow_event.isSet ():
            ...
            if byte == '\r' or byte == '\n':
                ...
                if ord (line[4:5]) < 49 or ord (line[4:5]) > 56: continue
